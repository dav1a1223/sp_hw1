...
int main(int argc, char** argv) {
    ...
    while (1) {
        watchset = savedset;
        select(maxfd + 1, &watchset, NULL, NULL, NULL);
        ...
        ret = handle_read(&requestP[conn_fd]);
            if (ret < 0) {
                fprintf(stderr, "bad request from %d %s\n", i, requestP[conn_fd].host);
                continue;
            }
            ...
            if (ret == 0) break;
            while (1) {
              read_lock(file_fd, 0, SEEK_SET, 0);
